abstract class Sektor {
	...
	protected employees: string[] = []; //da mogu da pristupe sve klase koje nasledjuju

	constructor(public naziv: string, private readonly sifraOdeljenja: number) {
		// this.naziv = naziv;
	...
	// abstract detaljanOpis: () => void;  moze i ovako
	abstract detaljanOpis(): void;

	describe(opis: string) {
...
class ITSektor extends Sektor {
	...
	constructor(public id: number, public admins: string[]) {
		super("IT Sektor", id);
...
class Accounting extends Sektor {
	...
	private constructor(public id: number, private reports: string[]) {
		super("Accounting", id);
	...
	override addEmployee(nameOfEmp: string): void {
